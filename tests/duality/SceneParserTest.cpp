...
TEST_F(SceneParserTest, Transforms) {
    ...
    root["scene"][0]["dataset"]["type"] = "geometry";
    for (int i = 0; i < 16; ++i) {
        root["scene"][0]["dataset"]["transforms"][0][i] = static_cast<float>(i);
    }
    root["scene"][0]["source"]["type"] = "download";
    root["scene"][0]["source"]["filename"] = "/some/file";
    ...
    auto transform = dataset.transforms()[0];

    ASSERT_EQ(0.0f, transform.m11);
    ASSERT_EQ(1.0f, transform.m12);
    ASSERT_EQ(2.0f, transform.m13);
    ASSERT_EQ(3.0f, transform.m14);

    ASSERT_EQ(4.0f, transform.m21);
    ASSERT_EQ(5.0f, transform.m22);
    ASSERT_EQ(6.0f, transform.m23);
    ASSERT_EQ(7.0f, transform.m24);

    ASSERT_EQ(8.0f, transform.m31);
    ASSERT_EQ(9.0f, transform.m32);
    ASSERT_EQ(10.0f, transform.m33);
    ASSERT_EQ(11.0f, transform.m34);

    ASSERT_EQ(12.0f, transform.m41);
    ASSERT_EQ(13.0f, transform.m42);
    ASSERT_EQ(14.0f, transform.m43);
    ASSERT_EQ(15.0f, transform.m44);
}
